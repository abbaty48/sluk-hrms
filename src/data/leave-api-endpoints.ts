...
export function hrmsLEAVE_ENDPOINTS(server: Application, getDb: () => TDatabase) {
    ...
    server.post("/api/leaves", (req: TAuthRequest, res: Response): void => {
        const db = getDb();
        const { staffId, leaveTypeId, startDate, endDate, reason }: TLeaveApplication =
            req.body;
    ...
    // Leave Pending
    server.get("/api/leaves/pending", (req: TAuthRequest, res: Response): void => {
        const db = getDb();
    ...
    server.get("/api/leaves/conflicts", (req: TAuthRequest, res: Response): void => {
        const db = getDb();
    ...
    server.post("/api/leaves/validate", (req: TAuthRequest, res: Response): void => {
        const db = getDb();
