...
export function hrmsATTENDANCE_ENDPOINTS(server: Application, getDb: () => Database) {
    ...
    server.post("/api/attendance/mark", (req: AuthRequest, res: Response): void => {
        ...
        if (!staffId || !date || !status) {
            res.status(400).json({ error: "Staff ID, date, and status are required" });
            return;
        }
    ...
    server.get("/api/attendance", (req: AuthRequest, res: Response): void => {
        ...
        const enrichedRecords = records.map((record) => {
            ...
            const department = staff ? db.departments.find((d) => d.id === staff.departmentId) : null;
    ...
    server.get("/api/charts/current-week-attendance", (_req: AuthRequest, res: Response): void => {
        ...
        const weekData = weekDates.map((date) => {
            ...
            const attendanceRate = totalStaff > 0
                ? parseFloat(((present + late + halfDay) / totalStaff * 100).toFixed(1))
                : 0;
        ...
        const weekSummary = {
            ...
            avgAttendanceRate: parseFloat(
                (weekData.reduce((sum, d) => sum + d.attendanceRate, 0) / weekData.length).toFixed(1)
            ),
    ...
    server.get("/api/attendance/department/:departmentId/stats", (req: AuthRequest, res: Response): void => {
        ...
        const stats = {
            ...
            attendanceRate: records.length > 0
                ? parseFloat((
                    (records.filter((a) => ["PRESENT", "LATE", "HALF_DAY"].includes(a.status)).length / records.length) * 100
                ).toFixed(2))
                : 0,
            avgWorkHours: records.length > 0
                ? parseFloat((records.reduce((sum, a) => sum + (a.workHours || 0), 0) / records.length).toFixed(2))
                : 0,
        };
    ...
    server.get("/api/attendance/today", (_req: AuthRequest, res: Response): void => {
        ...
        const stats = {
            ...
            attendanceRate: totalStaff > 0
                ? parseFloat((
                    (todayRecords.filter((a) => ["PRESENT", "LATE", "HALF_DAY"].includes(a.status)).length / totalStaff) * 100
                ).toFixed(2))
                : 0,
        };
    ...
    server.get("/api/attendance/report", (req: AuthRequest, res: Response): void => {
        ...
        const report = Array.from(staffAttendance.values()).map((data) => ({
            ...
            attendanceRate: data.totalDays > 0
                ? parseFloat(((data.present + data.late + data.halfDay) / data.totalDays * 100).toFixed(2))
                : 0,
            avgWorkHours: data.totalDays > 0
        ...
        res.json({
            ...
            summary: {
                ...
                avgAttendanceRate: report.length > 0
                    ? parseFloat((report.reduce((sum, r) => sum + r.attendanceRate, 0) / report.length).toFixed(2))
                    : 0,
            },
        });
    });
    ...
    server.get('/api/attendance/stats', (_req: AuthRequest, res: Response): void => {
        ...
        stats = attendance.reduce((_acc, attend) => {
            return {
                presentToday: attend.status === 'PRESENT' ? ++stats.presentToday : stats.presentToday,
                lateArrivals: attend.status === 'LATE' ? ++stats.lateArrivals : stats.lateArrivals,
                onLeave: attend.status === 'ON_LEAVE' ? ++stats.onLeave : stats.onLeave,
