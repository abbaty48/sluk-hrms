import type { TEnrichedStaff, TStaffDetails, TStaffPerDepartment, TStaffStatistics } from "@/types/staff-types";
import type { TLeaveBalance, TLeaveRequest } from "@/types/leave-management.types";
import type { TAuthRequest, TDatabase } from "@/types/types";
import type { Application, Response } from 'express'

export function hrmsSTAFF_ENDPOINTS(server: Application, getDb: () => TDatabase) {

    server.get("/api/staff/:id/employment", (req: TAuthRequest, res: Response) => {
        const db = getDb();
    ...
    server.get("/api/staff/:id/leave-balances", (req: TAuthRequest, res: Response) => {
        const db = getDb();
        const staffId = req.params.id;
    ...
    server.get(
        ...
        (req: TAuthRequest, res: Response): void => {
            ...
            const balance: TLeaveBalance[] = leaveTypes.map((type) => {
    ...
    server.get("/api/staff/:id/leaves", (req: TAuthRequest, res: Response): void => {
        const db = getDb();
    ...
    server.get("/api/staff/:id/dashboard", (req: TAuthRequest, res: Response): void => {
        const db = getDb();
        ...
        res.json({
            ...
            leavePercent: totalLeaveBalance.totalAllowed > 0 ?
                (totalLeaveBalance.totalUsed / totalLeaveBalance.totalAllowed) * 100 : 0,
            attendance,
            salary,
            recentLeaves,
        });
