...
export function hrmsANALYTICS_ENDPOINTS(server: Application, getDb: () => Database) {
    ...
    server.get("/api/analytics/staff-by-category", (req: AuthRequest, res: Response): void => {
        ...
        const data = [
            ...
            { name: "Non-Academic", value: categoryCount["Non-Teaching"], color: colors[3] },
    ...
    server.get("/api/analytics/monthly-leave-usage", (req: AuthRequest, res: Response): void => {
        ...
        for (let i = monthsCount - 1; i >= 0; i--) {
            ...
            const totalDays = monthLeaves.reduce((sum, leave) => {
                const start = new Date(Math.max(new Date(leave.startDate).getTime(), new Date(date.getFullYear(), date.getMonth(), 1).getTime()));
                const end = new Date(Math.min(new Date(leave.endDate).getTime(), new Date(date.getFullYear(), date.getMonth() + 1, 0).getTime()));
    ...
    server.get("/api/analytics/summary", (req: AuthRequest, res: Response): void => {
        ...
        const nonTeaching = staff.filter((s) => s.cadre === "Non-Teaching").length;
        const admin = staff.filter((s) => s.cadre !== "Teaching" && s.cadre !== "Non-Teaching").length;
        ...
        const rejectedLeaves = yearLeaves.filter((l) => l.status === "REJECTED").length;
    ...
    server.get("/api/analytics/department-performance", (_req: AuthRequest, res: Response): void => {
        ...
        const departments = db.departments.map((dept) => {
            const deptStaff = db.staff.filter((s) => s.departmentId === dept.id && s.status === "Employed");
            ...
            return {
                ...
                nonTeaching: deptStaff.filter((s) => s.cadre === "Non-Teaching").length,
                totalLeaves: deptLeaves.length,
                approvedLeaves,
                leaveApprovalRate: deptLeaves.length > 0
                    ? parseFloat(((approvedLeaves / deptLeaves.length) * 100).toFixed(1))
                    : 0,
            };
        });
    ...
    server.get("/api/analytics/year-over-year-growth", (_req: AuthRequest, res: Response): void => {
        ...
        const dataWithGrowth = growthData.map((current, index) => {
            ...
            const growthRate = previous.totalStaff > 0
                ? parseFloat((((current.totalStaff - previous.totalStaff) / previous.totalStaff) * 100).toFixed(2))
                : 0;
