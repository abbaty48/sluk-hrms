 ...
 *
 */
function LeaveReason({ reason }: { reason: string }) {
    const [isHover, setIsHover] = useState(false)
    return (
        <Popover open={isHover}>
            <PopoverTrigger onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
...
function TableRow({ id, status, startDate, staff, allowedDays, endDate, leaveType, reason }: TLeaveResponse) {
    ...
    const handleLeaveApprove = async (id: string) => {
        mutateApproval(id, {
            ...
            onError: () => {
                toast.error('Approval Failed ', { description: 'Failed to approved leave.' })
            }
        })
    }
    const handleLeaveRejection = async (id: string) => {
        mutateRejection(id, {
            ...
            onError: () => {
                toast.error('Rejection Failed', { description: 'Failed to reject leave.' })
            }
        })
    }
...
export function LeavePageTable() {
    ...
    const { data: leaves, isFetching, pagination, fetchNextPage, fetchPreviousPage } =
