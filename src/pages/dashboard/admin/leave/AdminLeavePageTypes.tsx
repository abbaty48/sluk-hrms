...
function LeaveTypes() {
  ...
  const [editingLeaveType, setEditingLeaveType] = useState<
    TLeaveType | undefined
  >();

  const { isPending: isDeletingLeaveType, mutateAsync: deleteLeaveTypeAsync } =
    useLeaveTypeDeleteAPI();
      ...
      deleteLeaveTypeAsync(id, {
        onSuccess: () => {
          toast.success("Deleting Successful.", {
            description: "Leave type has successfully been deleted.",
          });
        },
        onError: (error) => {
          toast.success("Deleting Failed.", {
            description: "Leave type has failed to be delete." + error.message,
            duration: 10000,
          });
        },
      });
...
function LeaveTypeCardSkeleton() {
  return (
    <Card className="p-4 mb-6">
      ...
      <div className="flex items-center gap-1 rounded-lg border border-border bg-muted/30 px-3 py-1.5 text-sm">
